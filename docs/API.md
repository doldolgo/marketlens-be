...
| `netDom` | **이 페어를 옮길 때 쓰는 네트워크** (국내 거래소 기준). null 이면 망을 확인하지 못한 것 |
| `depDom` / `wdDom` | **국내** 거래소의 입금 / 출금 가능 여부 (3-state, 아래 표) |
| `depFx` / `wdFx` | **해외** 거래소의 입금 / 출금 가능 여부 (3-state, 아래 표) |

**입출금 4개 필드는 `netDom` 네트워크에서의 상태다** — 코인 단위 값이 아니다.
코인 단위로만 보면 틀린다:

> 바이낸스 GRT 는 Arbitrum 출금이 열려 있어 코인 단위로는 "출금 가능"이다.
> 그러나 업비트는 GRT 를 **Ethereum 으로만** 받고 바이낸스의 Ethereum 출금은
> 막혀 있다. **옮길 수 없다.** 그래서 `wdFx` 는 `false` 다.

국내 거래소를 기준으로 잡는 이유는 국내가 지원하는 망이 곧 제약이기 때문이다
— 업비트는 코인의 379/384, 빗썸은 503/511 이 망이 하나뿐이다.

같은 코인이라도 국내 거래소에 따라 결과가 갈릴 수 있다:

```
SEI  업비트(Sei 네이티브)  ↔ 바이낸스 SEI 출금 막힘     → 못 옮김
SEI  빗썸(Sei EVM)         ↔ 바이낸스 SEIEVM 출금 열림  → 옮길 수 있음
```

네트워크 이름은 거래소마다 제각각이라(같은 Base 를 업비트 `BASENET`, 빗썸
`BASE_ETH`, 바이낸스 `BASE` 라 부른다) 코드 → 이름 순으로 맞춘다. **맞는지
확신할 수 없으면 `null`(확인 불가)로 둔다** — 다른 망을 같다고 잘못 맞추면
못 옮기는 경로를 "가능"이라고 말하게 되기 때문이다.
