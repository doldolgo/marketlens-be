...
DomQuery = Annotated[
    ...
    Query(
        description=(
            ...
            "생략하면 설정 기본값. 환율도 **이 거래소의 KRW-USDT 호가**를 쓴다 "
            "— 테더 프리미엄이 거래소마다 다르기 때문"
        ),
        examples=["upbit"],
    ),
]
...
환율은 `usdkrw_rate` 에 저장된 **그 국내 거래소의 KRW-USDT 최우선 호가**다.
방향에 따라 갈라 쓴다 — 김프는 원화로 USDT 를 사므로 `ask`, 역프는 받은
USDT 를 원화로 팔므로 `bid`. 테더 프리미엄은 실제로 치르는 비용이다.
