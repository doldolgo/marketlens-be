...
```
 ┌────────────── 수집 (외부를 부르는 유일한 경로) ────────────────────┐
 │                                                                   │
 │  POST /refresh ──▶ 업비트·빗썸·바이낸스 시세/호가 + KRW-USDT 환율   │
 │       │                                                           │
 │       ├─▶ market_snapshots  거래소×코인: 현재가·호가·입출금 (UPSERT)│
 │       ├─▶ usdkrw_rate   국내 거래소별 KRW-USDT 호가 (ask/bid)       │
...
- **환산 없이 저장** — 가격·호가는 그 거래소 통화 그대로 (업비트·빗썸 = KRW,
  바이낸스 = USDT). 원화 환산은 조회 시점에 `usdkrw_rate` 환율을
  곱해서 한다. 이 환율은 **은행 고시환율이 아니라 국내 거래소의 KRW-USDT
  최우선 호가**다 — 화면에 "암묵환율"로 표시되는 값이 이것이다. 김프는
  매도호가(ask), 역프는 매수호가(bid)를 쓴다. 각 방향에서 원화와 USDT 를
  실제로 맞바꿀 때 물리는 값이라서다. 기준 거래소는 `krw_reference_exchange`
  설정으로 고르고 기본값은 업비트다.
